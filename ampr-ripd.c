 ...
 *    2.2     30.May.2017    Enable sending of call@locator to my server for dynamic map creation
...
static char *usage_string = "\nAMPR RIPv2 daemon " AMPR_RIPD_VERSION " by Marius, YO2LOJ\n\nUsage: ampr-ripd [-d] [-v] [-s] [-r] [-i <interface>]  [-t <table>] [-a <ip|hostname|subnet>[,<ip|hostname|subnet>...]] [-p <password>] [-m <metric>] [-w <window>] [-f <interface>] [-e <ip>] [-F <interface>] [-E <ip>] [-x <system command>] [-g <gateway>] [ -L <callsign@locator>]\n";
...
void nl_debug(void *msg, int len)
{
	...
	for (rh = (struct nlmsghdr *)msg; NLMSG_OK(rh, len); rh = NLMSG_NEXT(rh, len))
	{
	    ...
	    if (0 == (rh->nlmsg_flags & NLM_F_MULTI)) break;
	}
    }
}
#endif
...
int main(int argc, char **argv)
{
	...
	while ((p = getopt(argc, argv, "dvsrh?i:a:p:t:m:w:f:e:F:E:x:g:L:")) != -1)
	{
		switch (p)
		{
			...
			if (optarg && optarg[0] == '-')
			{
			    fprintf(stderr, "ampr-ripd: option -%c needs an argument.\n", p);
			    return 1;
			}
			break;
		}
